...
{
	if ($args->theme_location == 'left_header_menu') {
		$start_item = '<li class="home-logo"><a href="' . home_url() . '"><img src="' . get_stylesheet_directory_uri() . '/assets/images/home.png" alt="home"></a></li>';
		$items = $start_item . $items;
	}
	return $items;
}


function pierogi_section_styles($is_pattern = false, $return_class = false)
{
	$pattern = get_field('background_pattern', (is_front_page()) ? get_option('page_on_front') : get_the_ID());
	$page_color = get_field('page_color', (is_front_page()) ? get_option('page_on_front') : get_the_ID());
	if ($page_color):
		$color = '#43322B';
		$class = 'brown';
	else:
	...
	if ($is_pattern):
		$style .= ($pattern) ? 'background-image: url(' . $pattern['url'] . ')' : '';
	endif;
	if ($return_class):
		$data = $class;
	else:
