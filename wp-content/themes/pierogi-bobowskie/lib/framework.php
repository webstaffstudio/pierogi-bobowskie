<?php
/**
 * Various functions required for pierogi_bobowskie-foundation to work properly
 *
 ...
 */
if ( ! class_exists( 'pierogi_bobowskie_Navwalker' ) ) :
	/**
	 * pierogi_bobowskie_Navwalker
	 */
	class pierogi_bobowskie_Navwalker extends Walker_Nav_Menu {
		/**
		 * start_lvl
		 *
	...
	if ( ! class_exists( 'pierogi_bobowskie_Mobile_Navwalker' ) ) :
		/**
		 * pierogi_bobowskie_Mobile_Navwalker
		 */
		class pierogi_bobowskie_Mobile_Navwalker extends Walker_Nav_Menu {
			/**
			 * start_lvl
			 *
			 * @param string  $output output
 ...
 */
if ( ! function_exists( 'pierogi_bobowskie_menu_fallback' ) ) :
	/**
	 * pierogi_bobowskie_menu_fallback
	 *
	 * @return void
	 */
	function pierogi_bobowskie_menu_fallback() {
...
if ( ! function_exists( 'pierogi_bobowskie_active_nav_class' ) ) :
	/**
	 * pierogi_bobowskie_active_nav_class
	 *
	 * @param array $classes classes
	 * @param mixed $item item
	 * @return array $classes classes
	 */
	function pierogi_bobowskie_active_nav_class( $classes, $item ) {
		if ( 1 === $item->current || true === $item->current_item_ancestor ) {
			$classes[] = 'active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'pierogi_bobowskie_active_nav_class', 10, 2 );
endif;

// 3 - Pagination: {@link https://codex.wordpress.org/Pagination}
if ( ! function_exists( 'pierogi_bobowskie_pagination' ) ) :
	/**
	 * pierogi_bobowskie_pagination
	 *
	 * @return void
	 */
	function pierogi_bobowskie_pagination() {
		global $wp_query;
 ...
 */
if ( ! function_exists( 'pierogi_bobowskie_active_list_pages_class' ) ) :
	/**
	 * pierogi_bobowskie_active_list_pages_class
	 *
	 * @param mixed $input input
	 */
	function pierogi_bobowskie_active_list_pages_class( $input ) {
		$pattern = '/current_page_item/';
		$replace = 'current_page_item active';
		$output  = preg_replace( $pattern, $replace, $input );
		return $output;
	}
	add_filter( 'wp_list_pages', 'pierogi_bobowskie_active_list_pages_class', 10, 2 );
endif;

// 4 - Comments tree: @link {https://codex.wordpress.org/Function_Reference/Walker_Comment}
if ( ! class_exists( 'pierogi_bobowskie_Comments' ) ) :
	/**
	 * pierogi_bobowskie_Comments
	 */
	class pierogi_bobowskie_Comments extends Walker_Comment {
		/**
		 * Tree type.
		 *
		 * @var string
